...
<div>
    <h4 style="color: #e94c3d">Số dư: {{ session()->get('currency')->symbol_left}}{{ number_format(App\Modules\Wallet\Models\Wallet::where(['user'=>Auth::user()->id, 'currency_id' => session()->get('currency')->id])->select('balance_decode')->first()->balance_decode, session()->get('currency')->decimal) }} {{ session()->get('currency')->symbol_right}}</h4>
</div>
